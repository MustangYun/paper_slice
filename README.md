...
### 버전 이력 (v8 → v9)
원본 v8은 Windows에서만 검증되어 있었습니다. 이후 빌드·실행·문서를 손봐
**macOS / Linux / Windows 동일 명령어로 돌아가게** 정리했습니다
(`.gitattributes` 라인엔딩 정규화, bash/PowerShell 스크립트 페어, Apple Silicon 자동 `--platform=linux/amd64`, 상대경로 `docker-compose.yml` 등).
v9 업데이트로 **CPU 자동 튜닝 · 페이지 청킹 · 모델 프리베이크 · 기본 포트 8100**
도 합쳐졌습니다 — [주요 기능](#주요-기능) / [CPU 전용 운영 가이드 (v9)](#cpu-전용-운영-가이드-v9) 참고.
